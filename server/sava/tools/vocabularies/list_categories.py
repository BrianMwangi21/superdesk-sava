...
async def list_categories(args, ctx: ToolContext) -> ToolResult:
    vocab = await superdesk.get_resource_service("vocabularies").find_one_async(req=None, _id="categories")
    items = (vocab or {}).get("items") or []
    categories = [{"qcode": i.get("qcode"), "name": i.get("name")} for i in items if i.get("is_active", True)]
